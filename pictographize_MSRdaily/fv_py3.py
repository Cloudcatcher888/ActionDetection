...
def likelihood_moment(x, gaussians, weights, k, moment):	
	...
	for i in range(0, len(weights)):
		probabilities += weights[i]*gaussians[i]
	probability = weights[k]*gaussians[k]
	ytk = probability / probabilities
	return x_moment * ytk

def f(a,i,x,gau,wei,k,j):
	return a + likelihood_moment(x, gau, wei, k, j)
...
def likelihood_statistics(samples, means, covs, weights):
	...
	for k in range(0, len(weights)):
		...
		for i in range(0, len(gaussians)):
			s0[k]+=likelihood_moment(samples2[i], gaussians[i],weights,k,0)
			s1[k] += likelihood_moment(samples2[i], gaussians[i], weights, k, 1)
			s2[k] += likelihood_moment(samples2[i], gaussians[i], weights, k, 2)
	return s0, s1, s2
